...
let rec interp_block (env : env) (blk : block) : value = assert false

(* Interprète un statement *)
and interp_stat (env : env) (stat : stat) : unit = assert false

(* Interprète un appel de fonction *)
and interp_funcall (env : env) (fc : functioncall) : value = assert false

(* Interprète une expression *)
and interp_exp (env : env) (e : exp) : value = assert false
