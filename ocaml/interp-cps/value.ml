...
type tkey = KInt of Int64.t | KString of string
...
and env = { globals : (name, t) Hashtbl.t; locals : (name, t) Hashtbl.t list }
...
and coroutine_status =
  ...
  | Suspended of (t -> unit)
(* k: continuation au moment de resume *)
...
let as_bool = function Bool b -> b | Nil -> false | _ -> true

let as_function = function
  | Function f -> f
  | v -> failwith (to_string v ^ ": not a function")

let as_coroutine = function
  | Coroutine t -> t
  | v -> failwith (to_string v ^ ": not a coroutine")
...
let mod_ v1 v2 =
  match (v1, v2) with
  | Int n1, Int n2 -> Int (Int64.rem n1 n2)
  | _, _ -> failwith ("attempt to perform 'mod' on a " ^ to_string v1 ^ " value")
...
let equal v1 v2 =
  ...
  | Int n1, Int n2 -> n1 = n2
  | Float f, Int n | Int n, Float f -> cmp_int_float n f = Some 0
...
let lt v1 v2 =
  match (v1, v2) with
  | Int n1, Int n2 -> n1 < n2
  | Float f1, Int n2 -> cmp_int_float n2 f1 = Some 1
  | Int n1, Float f2 -> cmp_int_float n1 f2 = Some (-1)
  | Float f1, Float f2 -> f1 < f2
  | _, _ ->
...
let le v1 v2 =
  ...
  | Int n1, Int n2 -> n1 <= n2
  | Float f1, Int n2 -> (
      match cmp_int_float n2 f1 with Some (0 | 1) -> true | _ -> false)
  | Int n1, Float f2 -> (
      match cmp_int_float n1 f2 with Some (0 | -1) -> true | _ -> false)
  | _, _ ->
      failwith ("trying to compare " ^ to_string v1 ^ " and " ^ to_string v2)

let lookup_scope env name =
  try List.find (fun local_scope -> Hashtbl.mem local_scope name) env.locals
  with Not_found -> env.globals

let lookup_ident env name =
  try Hashtbl.find (lookup_scope env name) name with Not_found -> Nil

let set_ident env name v = Hashtbl.replace (lookup_scope env name) name v
